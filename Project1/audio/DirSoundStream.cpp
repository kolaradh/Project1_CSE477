...
void CDirSoundStream::WriteFrame(short *p_audio)
{
   ...
   if(m_audiocnt >= 1996)
   {
      ...
      {
          DWORD status = 0;
          HRESULT hrS = m_pSoundBuffer->GetStatus(&status);
          if (SUCCEEDED(hrS))
          {
              // Only start if not already playing
              if ((status & DSBSTATUS_PLAYING) == 0)
              {
                  HRESULT hr = m_pSoundBuffer->Play(0, 0, DSBPLAY_LOOPING);
                  if (hr == DSERR_BUFFERLOST)
                  {
                      // Restore the buffer and try once more
                      if (SUCCEEDED(m_pSoundBuffer->Restore()))
                      {
                          // Refill a small chunk after restore
                          // (optional: write some zeros or the last audio chunk here)
                  ...
                  {
                      TRACE(L"[DirSound] Play failed: 0x%08X  status=0x%08X  filled=%lu\n",
                          hr, status, (unsigned long)m_bufferfilled);
                      // Don’t flip m_bufferplaying; we’ll try again when more data arrives.
                  }
              }
              else
              {
                  // Someone already started it; keep our flag in sync.
                  m_bufferplaying = true;
              }
          }
          else
          {
              TRACE(L"[DirSound] GetStatus failed: 0x%08X\n", hrS);
          }
      }
